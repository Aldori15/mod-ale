...
namespace LuaGlobalBot
{
    ...
    /**
     * Executes a synchronous SQL query on the playerbots database.
     * Returns an [ALEQuery] result object or nil if no results.
     *
    ...
    /**
     * Executes an asynchronous SQL query on the playerbots database.
     * Results are passed to a callback function.
     *
     ...
     *
     * @param string query : SQL query string
     * @param function callback : function(ALEQuery|nil)
     */
    int PlayerbotsDBQueryAsync(lua_State* L)
    {
        ...
        ALE::GALE->queryProcessor.AddCallback(PlayerbotsDatabase.AsyncQuery(query).WithCallback([L, funcRef](QueryResult result)
            {
    ...
    /** 
     * Executes a SQL statement on the playerbots database without returning results.
